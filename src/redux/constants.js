export const AUTH_USER = "AUTH_USER";
export const AUTH_USER_DATA = "AUTH_USER_DATA";
export const IS_AUTH_TRUE = "IS_AUTH_TRUE";
export const SET_USER_DATA = "SET_USER_DATA";
export const GET_USER_DATA_ID = "GET_USER_DATA";
export const SET_USER_ID = "SET_USER_ID";
export const UPLOAD_PHOTO_AVA_USER = "UPLOAD_PHOTO_AVA_USER";
export const GET_EVENTS = "GET_EVENTS";
export const SET_EVENTS = "SET_EVENTS";
export const SET_NEW_EVENT = "SET_NEW_EVENT";
export const GET_EVENT_PROFILE = "GET_EVENT_PROFILE";
export const SET_EVENT_PROFILE = "SET_EVENT_PROFILE";
export const DEL_EVENT = "DEL_EVENT";
export const IS_TOGGLE_LOADING = "IS_TOGGLE_LOADING";
export const ERR_EVENT = "ERR_EVENT";
export const SUCCESS_EVENT = "SUCCESS_EVENT";
export const CLEAR_TOGGLE = "CLEAR_TOGGLE";
export const EVENT_USER_NAME = "EVENT_USER_NAME";
export const LOGIN_OUT = "LOGIN_OUT";
export const IS_TOGGLE_LOADING_AUTH = "IS_TOGGLE_LOADING_AUTH";
export const ERR_AUTH = "ERR_AUTH";
export const CLEAR_TOGGLE_AUTH = "CLEAR_TOGGLE_AUTH";
export const IS_TOGGLE_DEL_EV_FALSE = "IS_TOGGLE_DEL_EV_FALSE";
export const ADD_USER_EVENT = "ADD_USER_EVENT";
export const ADD_USER_ID_EVENT = "ADD_USER_ID_EVENT";
export const DEL_USER_EVENT = "DEL_USER_EVENT";
export const DEL_USER_ID_EVENT = "DEL_USER_ID_EVENT";
export const UPLOAD_PHOTO_AVA_EVENT = "UPLOAD_PHOTO_AVA_EVENT";
export const SUCCESS_UPDATE_MEMBER_EVENT = "SUCCESS_UPDATE_MEMBER_EVENT";
export const SUCCESS_UPDATE_USER_DATA = "SUCCESS_UPDATE_USER_DATA";
export const GET_MY_EVENTS = "GET_MY_EVENTS";
export const REGISTRATION_USER = "REGISTRATION_USER";
export const FILTR_EVENTS = "FILTR_EVENTS";
export const SET_AUTH_LOGIN_ERROR = "SET_AUTH_LOGIN_ERROR";
export const SET_REGISTRATION_ERROR = "SET_REGISTRATION_ERROR"; 
export const SET_AUTH_DATA_USER_ERROR = "SET_AUTH_DATA_USER_ERROR"; 
export const SET_EVENT_ERROR = "SET_EVENT_ERROR";